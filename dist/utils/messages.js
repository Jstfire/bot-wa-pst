"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.INVALID = exports.WAITING = exports.WEB_BUSEL = exports.PUBLIKASI = exports.THANKS = exports.ADMIN_END = exports.ADMIN_JAM = exports.JADWAL_BUKA = exports.LOKASI = exports.STATISTIK_UMUM = exports.SUB_MENUS = exports.MAIN_MENU_NEXT = exports.MAIN_MENU = void 0;
exports.MAIN_MENU = `🎉 Selamat datang di *Pelayanan Statistik Terpadu (PST) Badan Pusat Statistik Kabupaten Buton Selatan* 🎉

Silakan pilih layanan yang Anda butuhkan dengan mengetik angka sesuai pilihan berikut:

1. 📚 Perpustakaan – Akses buku & referensi statistik
2. 📄 Rekomendasi Statistik – Ajukan surat rekomendasi survei
3. 🧑‍🏫 Konsultasi Statistik – Tanya data, metode, atau analisis
4. 📈 Statistik Umum – Info terkini Buton Selatan
5. 📘 Publikasi DDA – Data lengkap tingkat kabupaten
6. 📗 Publikasi KCDA – Data per kecamatan
7. 💬 Chat Admin – Hubungi petugas langsung

📌 Cukup ketik angka layanan (contoh: 3) untuk mulai.

Kunjungi buselkab.bps.go.id untuk data dan publikasi lainnya.

> _Pesan ini dikirim otomatis oleh Bot PST BPS Kabupaten Buton Selatan._ `;
exports.MAIN_MENU_NEXT = `Silakan pilih layanan yang Anda butuhkan dengan mengetik angka sesuai pilihan berikut:

1. 📚 Perpustakaan – Akses buku & referensi statistik
2. 📄 Rekomendasi Statistik – Ajukan surat rekomendasi survei
3. 🧑‍🏫 Konsultasi Statistik – Tanya data, metode, atau analisis
4. 📈 Statistik Umum – Info terkini Buton Selatan
5. 📘 Publikasi DDA – Data lengkap tingkat kabupaten
6. 📗 Publikasi KCDA – Data per kecamatan
7. 💬 Chat Admin – Hubungi petugas langsung

📌 Cukup ketik angka layanan (contoh: 3) untuk mulai.

Kunjungi buselkab.bps.go.id untuk data dan publikasi lainnya.

> _Pesan ini dikirim otomatis oleh Bot PST BPS Kabupaten Buton Selatan._ `;
exports.SUB_MENUS = {
    "1": `*📚 Menu Perpustakaan*

Temukan sumber informasi statistik yang Anda butuhkan melalui layanan kami:

1. *PST _Online_* 🌐 – Akses koleksi buku dan referensi statistik secara daring, kapan saja dan di mana saja
2. *Lokasi & Jadwal PST _Offline_* 📍🕒 – Informasi lengkap tentang lokasi dan waktu layanan perpustakaan fisik kami

99. 🔙 *Kembali ke menu utama* 🔁

💡 Ketik angka sesuai pilihan Anda untuk melanjutkan.`,
    "2": `*📄 Rekomendasi Statistik*

Layanan ini membantu Anda dalam mengurus surat rekomendasi kegiatan statistik sesuai prosedur resmi. Silakan pilih submenu berikut:

1. *Web Romantik* 🌐 – Ajukan permohonan rekomendasi statistik secara online melalui sistem Romantik BPS
2. *Chat dengan admin* 💬 – Konsultasikan langsung kebutuhan atau kendala Anda dengan petugas kami

99. *Kembali ke menu utama* 🔁

💡 Ketik angka sesuai pilihan Anda untuk melanjutkan.`,
    "3": `*🧑‍🏫 Konsultasi Statistik*

Butuh bantuan memahami data, metode, atau analisis statistik? Kami siap membantu! Silakan pilih cara konsultasi yang Anda inginkan:

1. *Chat dengan admin* 💬 – Konsultasi secara langsung melalui pesan dengan petugas statistik kami
2. *Bertemu offline* 📍 – Atur jadwal untuk bertemu langsung di kantor BPS Kabupaten Buton Selatan

99. *Kembali ke menu utama* 🔁

💡 Ketik angka sesuai pilihan Anda untuk melanjutkan.`,
    "6": `*📗 Menu Publikasi KCDA (Kecamatan Dalam Angka)*

Akses data lengkap dan terpercaya dari setiap kecamatan di Kabupaten Buton Selatan.
Silakan pilih publikasi yang ingin Anda lihat:

1. *Kecamatan Batu Atas* 📊
2. *Kecamatan Lapandewa* 📊
3. *Kecamatan Sampolawa* 📊
4. *Kecamatan Batauga* 📊
5. *Kecamatan Siompu Barat* 📊
6. *Kecamatan Siompu* 📊
7. *Kecamatan Kadatua* 📊

99. Kembali ke menu utama 🔁

💡 Ketik angka sesuai kecamatan yang ingin Anda akses.
`,
};
exports.STATISTIK_UMUM = `*📊 Statistik Umum Kabupaten Buton Selatan*

📘 Berdasarkan Publikasi Kabupaten Buton Selatan Dalam Angka 2025:
- *👥 Jumlah Penduduk:* 102.881 jiwa
- *📈 Indeks Pembangunan Manusia (IPM):* 68,04
- *💸 Persentase Penduduk Miskin:* 14,28%

📗 Berdasarkan Publikasi PDRB Menurut Lapangan Usaha 2020–2024:
- *💰 PDRB ADHB:* Rp3.826,60 miliar
- *💵 PDRB ADHK:* Rp2.320,67 miliar
- *🌾 PDRB ADHB Kategori Pertanian, Kehutanan, dan Perikanan:* Rp1.289,97 miliar
- *🌱 PDRB ADHK Kategori Pertanian, Kehutanan, dan Perikanan:* Rp750,42 miliar

📋 Berdasarkan Survei Angkatan Kerja Nasional (SAKERNAS) 2024:
- 🧑‍💼 Tingkat Pengangguran Terbuka (TPT): 3,48%

📝 *Catatan:*
* *_PDRB ADHB:* Produk Domestik Regional Bruto Atas Dasar Harga Berlaku_
* *_PDRB ADHK:* Produk Domestik Regional Bruto Atas Dasar Harga Konstan (tahun dasar 2010)_
`;
exports.LOKASI = `*📍 Lokasi Pelayanan Statistik Terpadu (PST) BPS Kabupaten Buton Selatan*

🗺️ Google Maps: `;
exports.JADWAL_BUKA = `*🕒 Jadwal Pelayanan:*

* 📅 *Senin – Kamis:* 07.30 – 16.00 WITA
* 📅 *Jumat:* 07.30 – 16.30 WITA
* ❌ *Sabtu & Minggu:* Libur

💡 Silakan kunjungi kami sesuai jam layanan untuk mendapatkan pelayanan data dan konsultasi statistik secara langsung. Kami siap membantu Anda! 🙌
`;
exports.ADMIN_JAM = `⏰ *Jam Operasional Admin:* 08.00 – 20.00 WITA
💬 Anda bisa berkonsultasi langsung dengan admin selama jam operasional.`;
exports.ADMIN_END = `Jika Anda sudah selesai berkonsultasi, *silakan ketik "00" untuk mengakhiri chat dan kembali ke menu utama.* 🔁
🙏 Terima kasih telah menggunakan layanan kami!`;
exports.THANKS = `*🙏 Terima kasih telah menggunakan layanan kami!*`;
exports.PUBLIKASI = `*📘 Berikut merupakan _file_ publikasi yang ingin Anda akses.*`;
exports.WEB_BUSEL = `*🌐 Kunjungi buselkab.bps.go.id untuk data dan publikasi lainnya.*`;
exports.WAITING = `*⏳ Mohon ditunggu sebentar...*`;
exports.INVALID = `*❗Pilihan tidak valid.*`;
